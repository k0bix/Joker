...
void gui_savesnap(void)
{
   ...
   gui_sendmsg(GUI_GREEN, (char *)"Screen saved to %s", filename);
}
...
void gui_togglesprites(void)
{
   ...
   gui_sendmsg(GUI_GREEN, (char *)"Sprites %s", option_drawsprites ? "displayed" : "hidden");
}
...
void gui_togglefs(void)
{
   ...
   if (machine->autoframeskip)
      gui_sendmsg(GUI_YELLOW,(char *) "automatic frameskip");
   else
      gui_sendmsg(GUI_YELLOW, (char *)"unthrottled emulation");
}
...
#define BLANK_CHAR 0x7F /* ASCII 127   [delta] */
   ...
   gui_sendmsg(GUI_ORANGE, (char *)"%ca %cb %cc %cd %ce %cext",
...
{
   char *types[3] = {(char *)"no", (char *)"lowpass", (char *)"weighted"};
   ...
   gui_sendmsg(GUI_ORANGE, (char *)"%s filter", types[filter_type]);
...
#define WAVEDISP_WIDTH 128
   ...
   if (NULL == vis_buffer)
   {
      ...
      gui_textbar((char *)"no sound", xofs + 40, yofs + 0x20 - 4, &small, GUI_RED, GUI_DKGRAY, BUTTON_UP);
   }
...
{
   /* Pretty it up a bit */
   gui_textbar((char *)"Pattern Table 0", 0, 0, &small, GUI_GREEN, GUI_DKGRAY, BUTTON_UP);
   gui_textbar((char *)"Pattern Table 1", 128, 0, &small, GUI_GREEN, GUI_DKGRAY, BUTTON_UP);
...
static void gui_updateoam(void)
{
   ...
   gui_textbar((char *)"Current OAM", 0, y, &small, GUI_GREEN, GUI_DKGRAY, BUTTON_UP);
